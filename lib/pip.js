...
function installRequirements(
  ...
  if (options.dockerizePip) {
    ...
    const bindPath = getBindPath(servicePath, true);

    // EDIT - Vince
    // on Windows running the Docker Toolkit (required for Windows 7)
    // process.env.HOME returns c:\{path} where the 'c:' is causing
    // docker issues because the ':' is being treated as a new
    // command line parameter.  I mofified the getBindPath method
    // to take a parameter to decide if it will test for the 
    // existence of serverless.yml

    // this is required to work with ssh ids mounting into the container as /root/.ssh/id_rsa
    const sshPath = getBindPath(`${process.env.HOME}/.ssh`);
    ...
    if (options.dockerSsh) {
      ...
      cmdOptions.push(
        ...
        `${sshPath}/id_rsa:/root/.ssh/id_rsa:z`
      );
      cmdOptions.push(
        '-v',
        // `${process.env.HOME}/.ssh/known_hosts:/root/.ssh/known_hosts:z`
        `${sshPath}/known_hosts:/root/.ssh/known_hosts:z`
      );
    ...
    // EDIT - Rob
    // all platforms use UID 0 within docker, as prior to v4.0.0
    // this is required to work with ssh ids mounting into the container as /root/.ssh/id_rsa
    cmdOptions.push('-u', '0')
    //
