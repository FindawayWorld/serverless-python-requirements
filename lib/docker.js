 ...
 * @return {boolean}
 */
function tryBindPath(bindPath, tryFindYml) {
  const options = [
    'run',
    '--rm',
    '-v',
  ...
  if(tryFindYml){
    options.push('ls', '/test/serverless.yml');
  }
 ...
 */
function getBindPath(servicePath, ifTryFindYml) {
  // Determine bind path
  ...
  for (let i = 0; i < bindPaths.length; i++) {
    ...
    if (tryBindPath(bindPath, ifTryFindYml)) {
      return bindPath;
    }
  }
